...
- Local re-verification on this checkout: full pytest run green, 612 tests (611 passed, 1 skipped, zero failures); ruff check and mypy clean on touched files. Follow-up: the +3 line shift stale-pinned the register (627, now a statement) and the shipped-register unit test plus the coverage gate failed closed as designed; re-pinned to 630, the same bounded-frame branch. The coverage gate failed closed mid-PR on a stale register pin (collectors.py 625) after the C4.3 +2 line shift and passed after re-pinning to the same bounded-frame branch at 627. No source changes since the green CI build; only this ledger update follows.
- Decision stays NO-GO.

...
Run offline preflight and a reviewed frozen-input dry-run, reconcile every ambiguous/partial delivery, then enable one scheduler. The dry-run must use `python -m meco_news --dry-run --frozen-input <reviewed-input.json> --json`; it does not collect live sources or mutate state. Verify a terminal event, health, backup, and no duplicate confirmed chunk. Never use mutable tags, overwrite the only database copy, run old/new binaries together, or force through unresolved state. Escalate to the release approver and record a regression test.

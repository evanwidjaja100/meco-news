...
class TestC35ZeroOutageDistinct(unittest.TestCase):
    def test_empty_success_vs_all_failed_distinct(self) -> None:
        ...
        _src = inspect.getsource(app_mod.run_once)
        ...
        if not has_empty_healthy:
            self.fail(
                "BUG REPRODUCED: health does not distinguish completed_empty — must be healthy for empty success but unhealthy for all_failed (C3.5)"
            )
    ...
    def test_all_sources_failed_is_retry_not_empty(self) -> None:
        ...
        if "all_sources_failed" not in src and "retry_wait" not in src:
            self.fail(
                "BUG REPRODUCED: health does not check all_sources_failed retry exhaustion — must be unhealthy after max attempts (C3.5)"
            )
    ...
    def test_empty_success_is_completed_empty(self) -> None:
        ...
        has_completed_empty_outcome = (
            src.count("completed_empty") >= 2 and 'outcome="completed_empty"' in src or "outcome='completed_empty'" in src
        )

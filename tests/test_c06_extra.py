...
class TestExtra(unittest.TestCase):
    ...
    def test_collectors_comprehensive(self):
        from meco_news.collectors import _bounded_text, _repair_xml, _entry_values
    ...
    def test_telegram_all(self):
        from meco_news.telegram import build_digest, validate_message, _clean_display, DEFAULT_MESSAGE_BYTES
        ...
        items = [
            NewsItem(title=f"t{i}", url=f"https://example.com/{i}", source="S", topic_label="T", relevance_reason="R") for i in range(3)
        ]
        res = build_digest(items, "MECO", "UTC", max_length=3900)
        self.assertGreater(len(res.messages), 0)

    def test_urls_all(self):
        from meco_news.urls import validate_url, same_or_allowed_redirect, sanitize_url_for_log
    ...
    def test_config_all(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            # Test invalid port
            bad = dict(base)
            bad["lease_ttl_seconds"] = 10
            p.write_text(json.dumps(bad), encoding="utf-8")
            with self.assertRaises(ConfigurationError):
                load_config(p)
            # Test trusted_domains with slash
            bad = dict(base)
            bad["trusted_domains"] = ["example.com/path"]
            p.write_text(json.dumps(bad), encoding="utf-8")
            with self.assertRaises(ConfigurationError):
                load_config(p)

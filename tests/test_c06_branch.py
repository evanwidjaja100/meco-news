...
class TestBranch(unittest.TestCase):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            with StateStore(p) as s:
                ...
                s.connection.execute(
                    "UPDATE outbox_chunks SET state='retry_wait', next_attempt_at='2099-01-01T00:00:00+00:00' WHERE delivery_id=?",
                    (d1.delivery_id,),
                )
                s.connection.commit()
                ...
                # Test due with past
                s.connection.execute(
                    "UPDATE outbox_chunks SET next_attempt_at='2000-01-01T00:00:00+00:00' WHERE delivery_id=?", (d1.delivery_id,)
                )
                s.connection.commit()
    ...
    def test_config_and_urls(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            bad = dict(base)
            bad["topics"] = base["topics"] + [base["topics"][0]]
    ...
    def test_ranking_and_preflight(self):
        ...
        items = [
            NewsItem(
                title=f"completely different headline number {i} about gas infrastructure project {i}",

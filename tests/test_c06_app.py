...
class TestAppCollectorsStorage(unittest.TestCase):
    ...
    def test_collectors_gdelt_and_google(self):
        from meco_news.collectors import _collect_gdelt, _collect_rss, SourceDataError
    ...
    def test_network_telegram_branches(self):
        from meco_news.telegram import TelegramClient, TelegramSendError, validate_message
        ...
        client = BoundedHTTPClient(CollectionLimits(max_redirects=0), NetworkPolicy(require_https=False), allow_private_for_tests=True)
    ...
    def test_ranking_and_config(self):
        ...
        items = [NewsItem(title=f"title {i}", url=f"https://example.com/{i}", source="S", published_at=datetime.now(UTC)) for i in range(10)]
        for it in items:
